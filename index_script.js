    ...
    function openDetail(index){
      ...
      document.getElementById("detailVisual").innerHTML = `
        <img class="detail-main-image" id="detailMainImage" src="${item.images[0]}" alt="${item.title}" />
        <div class="detail-thumbs">
          ${item.images.map((src, i) => `
            <button class="detail-thumb ${i === 0 ? "active" : ""}" onclick="setDetailImage(${i})" type="button">
              <img src="${src}" alt="${item.title} detail ${i + 1}" loading="lazy" />
            </button>
          `).join("")}
        </div>
      `;
    ...
    function updateNews(){
      document.getElementById("newsTrack").style.transform = `translateX(-${activeSlide * 100}%)`;
    }
    ...
    if(initialFilter){
      ...
      updateProductsHeading("All");
      renderProducts();
    }
    setInterval(nextHeroProduct, 5200);
    setInterval(nextNews, 7600);
