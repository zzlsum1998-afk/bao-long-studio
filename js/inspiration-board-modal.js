    ...
    const __baolongInspirationBoardMode = String(
      document.currentScript && document.currentScript.dataset
        ? document.currentScript.dataset.boardMode || ''
        : ''
    ).toLowerCase();
    let modalImages = [];
    let modalIndex = 0;
    ...
    function getBoardPreset(type, text){
      ...
      const isRender = lower.includes('render') || __baolongInspirationBoardMode === 'other';
