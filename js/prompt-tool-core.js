...
const silentLibrary = {
            ...
            "随机风格配色": ["minimal color palette illustration, dominated by rust brown (#833514), butter yellow (#F5E283), and navy blue (#3B5183), balanced color blocking, modern design composition, soft lighting, clean visual aesthetic", "soft pastel color palette, dominated by buttermilk cream (#FFF1B5), pastel blue (#C1DBE8), and old burgundy (#43302E), calm balanced tones, minimal composition, modern aesthetic, soft ambient lighting", "earth tone color palette, dominated by sage green (#788176), warm rusk beige (#CCB999), and deep pipe red (#5E1803), natural muted tones, harmonious color balance, contemporary visual composition", "nature-inspired color palette, dominated by olivine green (#9BAD50), columbia light blue (#B6CFE4), and van dyke brown (#393027), soft natural tones, balanced color harmony, modern visual composition", "neutral minimalist color palette, dominated by cadet gray (#92A0A1), soft isabelline off-white (#EEEAE7), and deep bistre brown (#3A2B22), subtle elegant tones, calm modern aesthetic", "Fresh natural palette, lime green and soft pink accents, deep dark forest green base, vibrant yet balanced atmosphere.", "Earthy Morandi palette, olive and reseda green tones, sandy dun beige, organic muted colors, wabi-sabi aesthetic.", "High-contrast warm palette, burnt rust orange, buttery yellow, deep navy blue, bold graphic color blocks, retro feel", "Warm afternoon mood, rich caramel brown, creamy papaya whip, soft baby blue, cozy and inviting architectural lighting", "vivid safety orange, clean flash white, dark slate teal, sleek contemporary materials, high energy", "obsidian black base, seashell off-white, denim blue textures, elegant minimalist design", "mossy bistre brown, white chocolate cream, deep red-violet, luxurious and moody atmosphere", "Yinmn blue, warm old lace, vibrant scarlet red, traditional and authoritative color scheme", "Spring garden palette, periwinkle and zaffre blue, bright citron yellow, floral-inspired colors, airy and luminous", "Quiet depth palette, moss green, soft old lace, vibrant iris purple, dreamy landscape lighting, mysterious and calm"],
            "日常人物活动": ["one or two people walking along the path in casual contemporary clothing, relaxed natural posture", "small groups of two or three strolling through the park, naturally spaced across the scene", "people sitting on benches and chatting, subtle hand gestures, informal social interaction", "a person reading quietly on a bench, casual seasonal clothing, calm body language", "two friends drinking coffee at an outdoor table, relaxed conversation, understated gestures", "a person walking a dog along the path, natural stride, everyday neighborhood atmosphere", "people pausing in the sunshine, loose casual clothing, unposed natural behavior", "scattered visitors resting in the public space, varied seated and standing poses"],
            "社交互动类": ["small groups of two or three having conversations, varied gestures and natural spacing", "neighbors greeting each other along the path, subtle eye contact and friendly body language", "people gathering around shared seating, informal conversation, mixed seated and standing poses", "friends interacting in the plaza, casual clothing, relaxed gestures, no staged posing", "a compact community gathering, people facing one another in small conversational clusters", "two people exchanging objects or information, clear but understated interaction"],
            "儿童活动": ["children playing on playground equipment in small groups, energetic varied poses, nearby supervising adults", "kids running and laughing across an open play area, natural movement, casual seasonal clothing", "children climbing play structures, varied body positions, safe supervised activity", "children drawing on the ground together, crouching and kneeling poses, focused interaction", "kids exploring planting and landscape features, curious gestures, small-group activity", "children playing with simple outdoor toys, lively but naturally distributed across the scene"],
            "运动类活动": ["one or two joggers moving along the path, athletic clothing, natural running stride", "cyclists passing through the site at different distances, casual helmets and active posture", "small groups exercising in an open area, varied stretching poses, comfortable spacing", "people practicing yoga on the lawn, calm balanced poses, loosely arranged mats", "people playing a casual ball game, dynamic movement, small-group interaction", "a skater moving through the plaza, clear motion and relaxed urban activity"],
            "休闲娱乐活动": ["small groups picnicking on the grass, blankets and simple food, relaxed seated poses", "people lying or sitting on the lawn, reading and listening to music, informal arrangement", "visitors watching a small outdoor performance, mixed seated and standing audience", "people taking photographs near landscape features, natural viewing direction and gestures", "a person painting or sketching outdoors, compact setup, focused seated posture", "friends sharing a quiet leisure moment under trees, casual clothing, subtle interaction"],
            "场地体验活动": ["visitors exploring the space along multiple paths, varied walking directions and viewing angles", "people moving through the plaza and pausing at key nodes, naturally distributed circulation", "small groups gathering under trees, using shade and seating, relaxed spatial behavior", "visitors observing the landscape from a viewpoint, bodies oriented toward the scenery", "people touching or examining architectural and landscape details, curious understated gestures", "users experiencing the architecture at different distances, mixed walking, standing and resting poses"],
        };
        ...
        function buildAdaptiveFinish(colorPrompt, lightPrompt) {
            const parts = [
                "clear site organization",
                "readable spatial hierarchy",
                "balanced composition",
                "professional landscape architecture presentation"
            ];
            // Only add fallback guidance when the user has not chosen that dimension.
            // This avoids forcing every style into low saturation / white space / diffuse light.
            if (!colorPrompt) parts.push("cohesive color palette");
            if (!lightPrompt) parts.push("balanced natural lighting");
            return parts.join(', ');
        }
        ...
        function update() {
            ...
            if(core) {
                const finish = buildAdaptiveFinish(s3, s7);
...
2. 关键词用逗号分隔，保持专业且精炼
3. 忠实保留用户选择的画面语言、色彩和光影，不要擅自统一为低饱和、留白或柔光；只有用户明确选择或语义兼容时才可使用这些词
4. 末尾使用不与风格冲突的专业收尾：clear site organization, readable spatial hierarchy, balanced composition, professional landscape architecture presentation, \n--v 6 ${arVal} --style raw --s 750
5. 不要重复相同含义的关键词，不要添加与已选色彩或光影相矛盾的描述`;
...
${selections}`;
            ...
            try {
                ...
                const s1val = document.querySelector('#s1 .card.active')?.dataset.val || '';
                const s2val = document.querySelector('#s2 .card.active')?.dataset.val || '';
                const s4val = document.querySelector('#s4 .card.active')?.dataset.val || '';
                const s5val = Array.from(document.querySelectorAll('#s5 .card.active')).map(el => el.dataset.val).filter(Boolean).join(', ');
                const s6val = Object.values(currentRandomPicks).join(', ');
                const s7val = document.querySelector('#s7 .card.active')?.dataset.val || '';
                const mockBase = [s1val, s2val, currentSelectedColor, s4val, s5val, s6val, s7val, manual, intent]
                    .filter(Boolean)
                    .join(', ');
                const finish = buildAdaptiveFinish(currentSelectedColor, s7val);
                const aiPrompt = `${mockBase}, ${finish}\n--v 6 ${arVal} --style raw --s 750`;
                // ── 占位模拟结束 ──
